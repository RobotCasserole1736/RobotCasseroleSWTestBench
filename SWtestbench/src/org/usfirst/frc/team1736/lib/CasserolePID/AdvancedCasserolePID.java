...
* Therefore, we did it ourselves. This is the Advanced flavor of our PID algorithms.
* We have no idea if it will actually be useful for FRC purposes, but the concepts
* are not beyond what we believe an advanced FRC student could understand.
* <br><br> 
* See the CasserolePID class documentation for more info on how different PID stuff
* should be utilized. This class wrappers/extends that base PID class with more 
* functionality. However, its fundamental usage shoudld be the same as the main class.
* <br><br>
* There are number of new features added by the Advanced flavor.
* <br><br>
* Gain Scheduling is a controls technique where the gains for a PID controller
* are modified at runtime based on some variable. 
* 
* 
*/
...
public abstract class AdvancedCasserolePID extends CasserolePID {
	 ...
	 * @param Gst_in Gain Scheduling Type (schedule PID gains on Setpoint, error, or nothing). If scheduling is requested, the add*GainSchedulePoint() methods must be called to set up the schedulers.
	 * @param IntMagMax_in Maximum allowable magnititude of the integrator (before applying gain). Will allow windup, but not pass on giant integral terms to the rest of the system. 
	 */
	AdvancedCasserolePID(double Kp_in, double Ki_in, double Kd_in, double Kf_in, double Kdf_in, double Kp2_in, GainScheduleTypes Gst_in, double IntMagMax_in) {
	...
	/**
	 * Add a new point to the Proportional Gain Scheduler. Will utilize a variable P
	 * gain depending on the present value of the scheduler variable. The scheduler variable
	 * is defined by the setting given to this class's constructor. Currently, scheduling
	 * gains based on Setpoint, Error, or some external value is supported.
	...
	/**
	 * Add a new point to the Integral Gain Scheduler. Will utilize a variable I
	 * gain depending on the present value of the scheduler variable. The scheduler variable
	 * is defined by the setting given to this class's constructor. Currently, scheduling
	 * gains based on Setpoint, Error, or some external value is supported.
	 * @param scheduler_var_value scheduler variable value at which to define a gain
	...
	/**
	 * Add a new point to the Derivative Gain Scheduler. Will utilize a variable D
	 * gain depending on the present value of the scheduler variable. The scheduler variable
	 * is defined by the setting given to this class's constructor. Currently, scheduling
	 * gains based on Setpoint, Error, or some external value is supported.
	 * @param scheduler_var_value scheduler variable value at which to define a gain
	...
	/**
	 * Set the gain scheduler variable value. Only useful if Gain Scheduling
	 * is being used, and it is set to the "External" type.
	 * @param val_in
	 */
